...
def readConfig():
    with open("./_login.json",'r', encoding="utf-8") as load_f:
...
if __name__ == "__main__":
    ...
    if(not checkPW(user, pw)):
        print("Input is not valid, please check file './_login.json' ")
                        ...
                        if button.get_attribute("textContent").find("新增")>= 0:
                            ...
                            for i in inputfileds:
                                if i.get_attribute("placeholder").find("请输入当天晨检体温") >= 0:
                                    i.click()
                                    i.send_keys(str(random.randint(360,367)/10.0))
